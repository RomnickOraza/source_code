...
class IssuanceController extends Controller
{
    ...
    {
        $issuances = Issuance::with('item')->paginate(6); // Paginate 10 per page
    ...
    {
        // Find the issuance by ID or return 404 if not found
        $issuance = Issuance::with('item')->findOrFail($id);  // Get the issuance along with its item details
    
        // Get all items for the dropdown (optional, if you want to allow changing the item)
        $items = Item::all();  // Get all items for the select dropdown
    
        return view('issuances.edit', compact('issuance', 'items'));  // Pass the issuance and items to the view
    }
